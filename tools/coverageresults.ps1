...
Write-Host "Consolidating coverage results"

$files = Get-ChildItem -Path $rootPath -Filter 'coverage.cobertura.xml' -File -recurse
...
foreach ($file in $files) {
    ...
    $destinationFilePath = "$($destinationPath)CoverageResults_$($counter).xml"
